---
title: "Rによる発現解析演習② ~ QCスキル：品質を見極めて適切に処理を施す"
...
```{r prepare_data, include=FALSE, message=FALSE}
# このチャンクでは必要ライブラリの読み込みと Airway データの準備を行います。
suppressPackageStartupMessages({
  library(SummarizedExperiment); library(DESeq2); library(limma); library(edgeR)
  library(airway); library(sva);library(AnnotationDbi); library(org.Hs.eg.db); library(apeglm)
  library(ggplot2); library(pheatmap); library(patchwork); library(tibble); library(dplyr); library(stringr)
  library(matrixStats); library(ggrepel); library(msigdbr)
})
# デフォルトテーマの設定（再設定しても良い）
theme_set(ggplot2::theme_bw(base_size = 12))
# Airway データの読み込み
data("airway")
se <- airway
colData(se)$dex <- relevel(factor(colData(se)$dex), "untrt")
...
遺伝子発現データ（RNA‑seq）は主に**カウントデータ**であり、読取り数はポアソンや負の二項などの**離散分布**に従う。カウントでは平均と分散が密接に関連するため、ライブラリサイズや構成の違いに応じた正規化と分散安定化が重要である。

たとえば RNA‑seq のクラスタ解析では TPM のように遺伝子長とシーケンス深度を補正した値を用い、差分解析では raw count に基づいた NB‑GLM を採用する。

一方、プロテオームやメタボロームでは測定されるのはイオン強度などの**連続値**であり、概ね正規分布に近いものの電離効率や保持時間に起因するばらつきを含む。

そのため強度データでは log 変換に加えて**分散安定化正規化 (VSN)** や quantile 正規化が採用され、VSN は技術反復間のばらつきを抑えるのに有効であると報告されている。

下図は、シミュレートしたカウントデータ（負の二項／ポアソン）と正規分布に従う連続強度データのヒストグラムの比較例である。

離散カウントでは 0 が多く裾が重い一方、連続強度はおおむね対称で中心付近に集中していることがわかる。

```{r dist_examples, echo=FALSE}
set.seed(1)
x_counts <- rpois(1000, lambda = 10)
x_cont   <- rnorm(1000, mean = 0, sd = 1)
par(mfrow=c(1,2))
hist(x_counts, col='skyblue', main='Poisson: カウント', xlab='counts')
hist(x_cont, col='salmon', main='正規分布: 強度', xlab='intensity')
par(mfrow=c(1,1))
```

### オミクス横断のワークフロー（概要）

以下のように、原データから差分解釈までの流れを段階的に整理すると全体像が掴みやすくなる。
...
-   **解釈と集合解析**：MA/Volcano/PCA/ヒートマップで差分を可視化し、GSVA/fgsea 等で経路レベルにまとめる。

```{r, message=FALSE,echo=FALSE,out.width="50%"}
knitr::include_graphics("rnaseq_diagram.png")
```
...
| DNAメチル化/ChIP など  | カウント（ビスルファイト）または強度 | NB-GLM や β-binomial モデルなど  | RLE/TMM、β-mixture、ログ変換    |

### 実例：生データと正規化後の比較

以下では、Airway データから最初の数サンプルの raw counts、RLE サイズ係数による正規化、VST 変換の違いを簡単に比較する。`prepare_data` チャンクで `se` と `cts` は読み込まれている。

```{r example_norm, dependson="prepare_data"}
# 元データ（行: 遺伝子, 列: サンプル）から最初の3遺伝子・3サンプルを抽出
head(cts[1:3, 1:3])

# ---- 前処理：logCPM / size factors / VST -----------------------------------
logCPM <- edgeR::cpm(cts, log = TRUE, prior.count = 1)

# サイズ係数推定と正規化
dds_example <- DESeq2::DESeqDataSetFromMatrix(cts, colData = colData(se), design = ~1)
dds_example <- DESeq2::estimateSizeFactors(dds_example)
cts_norm <- t(t(cts) / sizeFactors(dds_example))

# VST変換
vst_example <- DESeq2::vst(dds_example, blind = TRUE)
vst_mat     <- SummarizedExperiment::assay(vst_example)

# ---- ① RLE（箱ひげ）: 正規化・分散安定化の効果 -----------------------------
rle_raw <- sweep(logCPM, 1, matrixStats::rowMedians(logCPM), FUN = "-")
rle_vst <- sweep(vst_mat, 1, matrixStats::rowMedians(vst_mat),   FUN = "-")

df_rle <- rbind(
  data.frame(value = as.vector(rle_raw),
             sample = rep(colnames(rle_raw), each = nrow(rle_raw)),
             set = "Raw (logCPM)"),
  data.frame(value = as.vector(rle_vst),
             sample = rep(colnames(rle_vst), each = nrow(rle_vst)),
             set = "VST")
)

p_rle <- ggplot(df_rle, aes(x = sample, y = value)) +
  geom_boxplot(outlier.size = 0.2) + coord_flip() +
  facet_wrap(~ set, ncol = 2) +
  labs(x = NULL, y = "RLE（遺伝子中央値差）",
       title = "RLEの比較：正規化・分散安定化で中央値が0付近に整列")

# ---- ② PCA: クラスタ構造の改善 --------------------------------------------
meta <- as.data.frame(colData(se)) |> tibble::rownames_to_column("sample")

pc_raw <- prcomp(t(logCPM), scale. = FALSE)$x[, 1:2]
pc_vst <- prcomp(t(vst_mat), scale. = FALSE)$x[, 1:2]

pca_raw_df <- tibble::as_tibble(pc_raw, rownames = "sample") |>
  dplyr::left_join(meta, by = "sample")
pca_vst_df <- tibble::as_tibble(pc_vst, rownames = "sample") |>
  dplyr::left_join(meta, by = "sample")

p_pca_raw <- ggplot(pca_raw_df, aes(PC1, PC2, color = dex, shape = cell)) +
  geom_point() + labs(title = "PCA（粗い正規化：logCPM）")

p_pca_vst <- ggplot(pca_vst_df, aes(PC1, PC2, color = dex, shape = cell)) +
  geom_point() + labs(title = "PCA（VST後）")

# ---- ③ 平均–標準偏差：VSTでばらつきが均質化 -------------------------------
df_ms <- rbind(
  data.frame(mean = rowMeans(logCPM),
             sd   = matrixStats::rowSds(logCPM),
             set  = "Raw (logCPM)"),
  data.frame(mean = rowMeans(vst_mat),
             sd   = matrixStats::rowSds(vst_mat),
             set  = "VST")
)

p_ms <- ggplot(df_ms, aes(mean, sd)) +
  geom_point(alpha = 0.2, size = 0.5) +
  geom_smooth(se = FALSE, method = "loess") +
  facet_wrap(~ set, ncol = 2) +
  labs(x = "遺伝子平均", y = "遺伝子標準偏差",
       title = "平均–標準偏差の関係：VSTで分散が平均に依存しづらくなる")

# ---- レイアウト（patchwork） ------------------------------------------------
(p_rle) / (p_pca_raw + p_pca_vst) / (p_ms)

```

このように、正規化と分散安定化によってデータのスケールと分布が整うことがわかる。実務では各 P の手順を繰り返し適用して、最終的な差分解析に備える。
...
配列読取りは「分子の**離散サンプリング**（counts）」であり、読取り数の総量差（**ライブラリサイズ・構成バイアス**）、断片化と配列特性に由来する**位置・塩基依存バイアス**、割当ての**不確実性**（多重割当・救済割当）を必然的に生む。

したがって、**盲目的（blind）に**(i)身元整合、(ii)信号健全性、(iii)系統偏り、(iv)総量/分布整合、(v)再現性、の順で**可視化→最小補正→共通尺度化**し、解析用アーティファクトを**凍結**して引き渡すのが核となる。
...
> **ワークフロー例**：ピーク検出/整列 → **QC時系列補正（P3）** → **スケール/分布補正（P4；blind）** → 再確認（P2--P4） → **再現性確認（P5）** → **引き渡し（アーティファクト凍結）**。 **DDA と DIA**：DDA は取得の確率性・欠測（MNAR）に配慮、DIA はライブラリ品質・整列（`DIAlignR`）の妥当性が鍵。 **TMT/iTRAQ**：参照チャネル基準で比を安定化し、比圧縮に注意（`MSstatsTMT`）。**LFQ**：分位/VSNと外れ頑健な集約（`MSstats`）。**メタボロミクス**：**QC-RLSC→PQN/内標→log/glog**が定石。
...
```{r p1_identity, dependson="prepare_data"}
# --- P1: 同定・整合性 ------------------------------------------------------
# サンプルIDの一意性
