...
# Detect feature (row) column: prefer 'gene' if present, else last column
...
# Ensure meta has sample_id and reorder to match counts' columns using BASE subsetting
...
# Build counts matrix using BASE operations (avoid column_to_rownames & dplyr::slice conflicts)
cts_df <- as.data.frame(cts_tbl, stringsAsFactors = FALSE)
# Reorder columns to feature + sample_cols
...
```{r make-se}
col_data <- S4Vectors::DataFrame(as.data.frame(meta_tbl))
