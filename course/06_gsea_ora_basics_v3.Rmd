...
上位遺伝子（例：**FDR < 0.05 且つ log2FC > 1**）を **SYMBOL** で抽出し、Hallmark の ORA を実行します。**母集団（universe）**は DGE 表に現れる **全SYMBOL** に設定します。
...
if (has_msigdbr && has_cp) {
  ...
  d_all <- dge2 |> filter(!is.na(SYMBOL)) |> distinct(SYMBOL, .keep_all = TRUE)
  universe <- d_all$SYMBOL
  ...
  if (length(up) >= 5) {
    ora_h <- clusterProfiler::enricher(gene = up,
                                       universe = universe,
                                       TERM2GENE = TERM2GENE,
                                       pAdjustMethod = "BH",
                                       pvalueCutoff = 0.05,
    ...
    res_df <- as.data.frame(ora_h)
    if (nrow(res_df) > 0) {
      out_ora <- file.path(dir_tab, "06_ora_hallmark.csv")
      readr::write_csv(as_tibble(res_df), out_ora)

      if (has_enrich) {
        print(enrichplot::dotplot(ora_h, showCategory = min(15, nrow(res_df))))
      } else {
        print(head(res_df, 15))
      }
    } else {
      cat("ORA: 有意な項目が 0 件でした（BH 0.05）。しきい値や gene set・universe の設定を調整してください。\n")
      # 参考: 名目 p 値ベースの上位表示（プロットは行わない）
      if (is.data.frame(ora_h@result) && nrow(ora_h@result) > 0) {
        print(head(ora_h@result, 10))
      }
    }
  } else {
    cat("閾値に合致する上位遺伝子が少ないため、Hallmark ORA をスキップしました（>=5 必要）。\n")
  }
} else {
...
if (has_cp && has_anno) {
  ...
  if (length(up) >= 5) {
    ...
    if (nrow(res_go) > 0 && has_enrich) {
      print(enrichplot::dotplot(ego, showCategory = min(15, nrow(res_go))))
    } else if (nrow(res_go) > 0) {
      print(head(res_go, 15))
    } else {
      cat("GO ORA: 有意な項目が 0 件でした。\n")
    }
  } else {
    cat("閾値に合致する上位遺伝子が少ないため、GO ORA をスキップしました（>=5 必要）。\n")
  }
} else {
