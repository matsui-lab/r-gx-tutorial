...
if (ok) {
  pheatmap::pheatmap(R, annotation_col = data.frame(group = colData(se)$group),
...
# クラスタリング樹形図
hc <- hclust(dist(t(logcpm)), method = "complete")
plot(hc, main = "Hierarchical clustering (log2-CPM)")
```
...
plot(pc$x[,1], pc$x[,2],
     ...
     main = "PCA of log2-CPM")
legend("topright", legend = levels(colData(se)$group),
       col = seq_along(levels(colData(se)$group))+1, pch = 16)
```
