...
# 4) 経路定義：MSigDB Hallmark（ヒト）  ※ base pipe でも安全な書き方
h_tbl <- msigdbr(species = "Homo sapiens", category = "H") |>
  dplyr::transmute(gs_name, gene_symbol = toupper(gene_symbol))
h <- split(h_tbl$gene_symbol, h_tbl$gs_name)
...
print(head(dplyr::select(fg, pathway, NES, size, padj), 10))
```
...
# 上位シグネチャ：上位300を採用（教材として簡単）
top_up <- names(ranks)[ranks > 0][1:min(300, sum(ranks > 0))] |> na.omit()
...
if (requireNamespace("enrichplot", quietly = TRUE)) {
  ...
  .msg("[NOTE] enrichplot が無いので、表を表示します。")
  print(head(as_tibble(ora_h@result), 15))
}
```
